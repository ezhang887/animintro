...
class MFCCDataset(Dataset):
    ...
    def __init__(self, audio_dir, label_dir, device):
        ...
        self.mfcc_transform = torchaudio.transforms.MFCC(sample_rate=8000, log_mels=True)
    ...
    def __getitem__(self, idx):
        ...
        padded_audio = pad_audio(waveform, self.max_length)
        mfcc_audio = self.mfcc_transform(padded_audio.mean(axis=0)).unsqueeze(0)

        mfcc_audio -= mfcc_audio.mean()
        mfcc_audio /= mfcc_audio.std()
        mfcc_audio = mfcc_audio.to(self.device)

        return mfcc_audio, self.labels[idx]
